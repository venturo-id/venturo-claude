...
3) Present the discovered test files to the user using an interactive AskQuestion that **allows multiple selections** (multi-select). The user can select one or more test files to run. Display each test file as an option with format: `<feature>/<filename>`. Include an "All" option at the top to select all test files.
