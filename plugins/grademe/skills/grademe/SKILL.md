...
**v0.6.0 (anti-gaming):** bobot rubrik digeser (planning 20→15, context 20→15, delegation 15→18, verification 15→20, token_efficiency 10→12; decomposition 15 & documentation 5 tetap; total 100). `misses` kini WAJIB ≥2 di SETIAP sesi → `total_score` efektif maks **98** (aturan lama "misses kosong ↔ ≥95" dihapus total). Grader membaca `score_caps` digest sebagai hard gate (sesi <15 mnt/<5 user turn → cap 85; tanpa friksi tertangani → cap 89) dan menganalisa `first_user_prompt`+`work_evidence` untuk mendeteksi prompt rubric-engineered (sesi pameran). Rubrik dipertajam: satu artefak tak boleh jadi bukti High untuk planning DAN context sekaligus. `digest.py` exit non-zero → **STOP TOTAL**, tak ada fallback (jalur "cerita sesi manual" ditutup). `scripts/validate.py` (Task 2) jadi gerbang final wajib sebelum presentasi/upload. Payload: +`first_user_prompt`/`work_evidence`/`score_caps` (disalin persis dari digest), `grademe_version` → `0.6.0`.
...
3. **Preprocess** (main agent, sebelum dispatch — jangan skip, ini yang membuat grading hemat token):
   ...
   - **HARD-STOP (v0.6.0) — `digest.py` exit non-zero (exit 2 = harness bukan Claude Code JSONL; exit lain = file rusak/kosong) → grademe STOP TOTAL.** Sesi TIDAK BISA dinilai. DILARANG keras: menilai dari memori percakapan ini, meminta user menceritakan sesinya, menerima/mengarang telemetri manual, membangun payload, atau upload apa pun. Pesan ke user: "Transcript tidak bisa dibaca (harness tidak didukung / file rusak) — sesi ini tidak bisa dinilai." Tidak ada fallback, tidak ada jalur alternatif.
...
5. **Validate** returned JSON (lihat Validation): pre-check LLM ringan dulu, LALU gerbang final `validate.py` (WAJIB — keputusan akhir ada di sini). Tulis JSON grader ke `/tmp/grademe_submission_${TAG}.json` (langkah upload §1) lalu jalankan:
   ```bash
   ...
   ```
   Exit 0 → lolos, lanjut. Exit non-zero (pelanggaran dicetak ke stderr sbg `VALIDATION FAIL: ...`) → re-dispatch grader dengan pesan pelanggaran itu sebagai konteks tambahan (maks 2 percobaan ulang), re-validate tiap kali. Masih gagal setelah 2 ulang → STOP dengan pesan jelas ke user. **TIDAK PERNAH mempresentasikan atau upload hasil yang gagal `validate.py`.**
...
**NO-DOUBLE-CREDIT RULE (v0.6.0):** a single artifact (e.g. a rich opening prompt) may NOT be the High-band evidence for two dimensions at once. To score `planning` AND `context` both high you must cite DIFFERENT evidence for each — plan-gate/sequencing for planning, concrete references spread across many prompts for context. If the only strong evidence is one opening prompt, at most one of the two can be High.

| Dimension (max) | Low band | Mid band | High band |
|---|---|---|---|
| planning (15) | 0–4: no plan; user dives straight into "build X" | 5–10: sequencing partial/reactive; plan emerges mid-work. Plan prompted reactively mid-session (after exploration/work began) lands here (max 10) even if approved pre-code | 11–15: plan gate from the session's outset — ExitPlanMode tool_use OR explicit plan requested in the opening prompt, approved BEFORE execution. Scores SEQUENCING only (plan-before-execute + quality of the approved plan), not richness of references |
| context (15) | 0–4: vague prompts, no files/docs referenced | 5–10: concrete references (files, docs, constraints, examples) present but thin or only in the opener | 11–15: concrete references spread across MANY prompts THROUGHOUT the session. **Explicit rule: however rich the opening prompt, if later prompts are context-poor, context is capped at mid band** — a great opener alone is not High |
...
| delegation (18) | 0–4: no leverage; only basic direct tools (Bash/Read/Write/Edit), user pastes what tools could fetch | 5–10: some leverage but ritual/unconsumed — dispatch whose output is never used, decorative skill, or good MCP use without any apt dispatch | 11–18: distinct+consumed subagent dispatches, and/or a relevant user-initiated skill that shaped the work, and/or deliberate MCP/plugin tooling acted on |
| verification (20) | 0–6: no test/build/run commands at all | 7–13: some Bash test/build tool_use, failures not followed up, OR results accepted blind ("ok mantap") without shown evidence | 14–20: test/build/run commands with tool_results checked AND failures acted on AND acceptance rests on shown evidence (test output read, `git diff` reviewed) — not blind approval. Watch for `suppressed:true` on the Bash tool_use (`\|\| true` / `; exit 0` / `--no-verify`): a suppressed test is NOT a passing test → caps this dimension at mid band, recorded as a miss |
| token_efficiency (12) | 0–4: repeated pasted content, redundant re-reads, bloated prompts | 5–8: minor repetition/waste | 9–12: lean prompts, no repeated pastes, targeted reads |
...
DELEGATION SCORING (18) — score three components from `tool_usage`, SUM them, then cap at 18. The Low/Mid/High row above is the summed outcome; these components are how you get there.

- **D1 — Subagent orchestration (0/2/5/7).**
  ...
  - `5`: ≥1 dispatch whose output is demonstrably consumed in a later turn (a later plan, Edit, Bash, or user instruction references its findings; the report has real `result_chars` and `total_tool_use_count > 1`).
  - `7`: ≥2 **distinct** consumed dispatches (`distinct_dispatches ≥ 2` and each consumed), OR an apt `subagent_type` matched to the job (`Explore` for mapping, `Plan` for design, `general-purpose` for execution), OR parallel fan-out (`parallel_turns ≥ 1`) followed by a synthesis turn.
...
- **D2 — Skill usage (0/2/5/6).**
  ...
  - `5`: `skills[].attributed_turns ≥ 5` AND the work visibly follows the skill's procedure.
  - `6`: `skills[].user_initiated == true` (the PARTICIPANT invoked it via `<command-name>`, not agent self-rescue) AND it shaped the outcome.
- **D3 — Specialized tooling (0/3/5).**
  - `0`: only Bash/Read/Write/Edit (`tool_usage.mcp.total_calls == 0`).
  - `3`: MCP/plugin tools (`tool_usage.mcp.servers`) used where a Bash workaround would have been clumsier.
  - `5`: MCP/plugin chosen deliberately over a manual path, with results acted on in a later turn.

**Floor rule: a session with ZERO skills AND ZERO subagents (`skills` empty AND `dispatches == 0`) CANNOT exceed 7/18 on delegation** — even with flawless direct tool use — because D1 and D2 are both 0 and only D3 (≤5) plus the base can apply. State this constraint if it binds.
...
SCORE CAPS (hard gates from digest) — read `score_caps` in the digest. **`total_score` MUST NOT exceed `score_caps.cap`.** If `score_caps.reasons` is non-empty, EACH reason becomes a `miss` with a plain explanation:
- `short_session→85` — "sesi <15 menit / <5 user turn — terlalu pendek untuk membuktikan praktik vibe coding matang → cap 85".
- `no_friction→89` — "tidak ada friksi (error/revisi plan) yang tertangani — sesi tanpa hambatan tidak kredibel sempurna → cap 89".
Skor 90+ menuntut bukti friksi yang ditangani baik: test gagal→diperbaiki, plan direvisi, atau error di-debug (cek `work_evidence.error_events`/`errors_followed_up`/`plan_revisions`/`friction_present`). Tanpa itu, jangan naik ke pita 90+.

SYNTHETIC-PROMPT DETECTION (rubric-engineered) — analyse `first_user_prompt` together with `work_evidence`. Bila prompt pembuka secara sistematis mengenumerasi poin-poin rubrik sekaligus (plan-gate + file refs + delegasi subagent + verifikasi + dokumentasi, gaya checklist/borongan) DAN sesi pendek-mulus (`work_evidence.friction_present == false` / turns sedikit) → indikasi kuat prompt hasil reverse-engineering rubrik. Record miss "indikasi prompt rubric-engineered (sesi pameran, bukan sesi kerja)" DAN batasi `planning` + `context` maks mid band. **Pembeda eksplisit: prompt pembuka kaya yang ORGANIK dengan sesi kerja panjang + friksi nyata TIDAK kena penalti ini — pembedanya adalah `work_evidence` (friksi + durasi + turns), bukan kualitas prompt-nya.**
...
- **planning** — Low: opening prompt is the whole spec, `"bikinin health check dong"`, code by 4th tool call. Mid (max 10): `"tunggu, jelaskan dulu rencanamu sebelum mulai ngoding"` — plan requested mid-flight after work began. High (11–15): `"Masuk plan mode dulu — jangan ngoding sebelum plan aku approve"` in the opening prompt, `ExitPlanMode` approved before any code.
- **context** — Low: `"perbaiki bug-nya"` tanpa satu pun file/doc. Mid (7–8/15): prompt pembuka kaya referensi tapi prompt-prompt lanjutan miskin konteks (`"lanjut"`, `"oke gas"`). High (11–15): referensi konkret (file/doc/constraint) konsisten di ≥3 prompt berbeda sepanjang sesi.
...
- **synthetic prompt** — opening prompt mengenumerasi seluruh rubrik (plan-gate+file refs+subagent+verifikasi+docs) tapi `work_evidence.friction_present:false`, `user_turns:3`, `duration_minutes:6` → `score_caps.cap:85`, planning & context ditahan mid band, miss "prompt rubric-engineered".

EVIDENCE RULE (contractual): every item in `misses` MUST quote or reference a concrete transcript event — short quote, or tool name + what happened. No evidence → the item may not appear. Scores without evidence are invalid. Each miss MUST cost ≥1 point in its dimension. Cite only from verbatim fields (`text`, `uuid`, tool_use `id`, Bash `command`) — never from lossy summaries.

MISSES ≥2 (v0.6.0): tidak ada sesi vibe coding yang sempurna. `misses` WAJIB berisi MINIMAL 2 item di SETIAP sesi, berapa pun skornya — grader yang tidak menemukan 2 kekurangan belum membaca digest dengan cermat. Karena tiap miss memotong ≥1 poin di dimensinya, konsekuensinya `total_score` maksimum efektif **98**. **`total_score` > 98 adalah output invalid.** (Aturan lama "misses kosong ↔ ≥95" dihapus — abaikan bila terlihat di memori.)
...
- Each breakdown value ≤ its max (15/15/15/18/20/12/5); values sum to `total_score`.
- `misses` berisi ≥2 item dan `total_score` ≤ 98. Violation → re-dispatch. (Ini pre-check ringan; gerbang final `validate.py` di langkah 5 yang menegakkan penuh — 7-key int breakdown, cap dari `score_caps`, kesamaan verbatim field forensik, dsb.)
...
After the fenced JSON, add one provenance line in the narrative: transcript path, sessionId, `session_name`, discovered vs explicitly passed (`sumber: sesi-aktif (env)` / `sumber: sesi-aktif (nonce)` / `sumber: manual (--transcript)`), file mtime, line count. Bila upload aktif, tambahkan satu kalimat disclosure: prompt pertama sesi (`first_user_prompt`) ikut terkirim ke leaderboard untuk keperluan audit kecurangan.
...
1. **Build submission body** dari JSON hasil grading — contract fields lama, ditambah:
   ...
   - **v0.6.0 — salin PERSIS dari digest sebagai field top-level:** `first_user_prompt` (prompt pertama verbatim), `work_evidence`, `score_caps`, `signal_availability`. `validate.py` menolak submission bila salah satu berbeda dari digest, jadi jangan edit/normalisasi. **Semua 9 field forensik yang diwajibkan `validate.py` (`FORENSIC_FIELDS`: `transcript_meta`, `usage_totals`, `type_counts`, `tool_usage`, `work_evidence`, `score_caps`, `first_user_prompt`, `session_id`, `signal_availability`) WAJIB disalin verbatim dari digest — hilang satu saja, submission ditolak gerbang validate.py.**
   ...
   - `grademe_version`: string literal `"0.6.0"` — samakan dengan `version` di `.claude-plugin/plugin.json`. Ini yang membuat BE bisa membedakan "peserta pakai skill lama" dari "digest gagal menghasilkan data".
